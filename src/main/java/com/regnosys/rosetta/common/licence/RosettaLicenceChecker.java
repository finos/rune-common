...
public class RosettaLicenceChecker {
    ...
    public static RosettaLicence check(Path publicKeyPath, List<Path> applicationLicencePaths, String applicationName, Action callback) {
        ...
        if (matchingLicences.isEmpty()) {
            ...
            callback.checkFailed("LICENCE FOR '" + nonMatchingLicences.stream().collect(Collectors.joining(",")) + "' CANNOT BE APPLIED TO '" + applicationName + "'.");
            return null;
        }

        List<RosettaLicence> validLicences = matchingLicences.stream()
                .filter(l -> l.getExpiry().isAfter(LocalDate.now()) || l.getExpiry().equals(LocalDate.now())).collect(Collectors.toList());
        ...
        if (validLicences.isEmpty()) {
            ...
            callback.checkFailed("LICENCE FOR '" + applicationName + "' EXPIRED ON " + nonValidLicences.stream().map(Object::toString).collect(Collectors.joining(",")) + ".");
            return null;
        }
