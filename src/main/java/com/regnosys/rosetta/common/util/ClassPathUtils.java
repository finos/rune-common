...
public class ClassPathUtils {
	...
	public static List<Path> findPathsFromClassPath(Collection<String> classPathDirectories, String includeRegex, Optional<String> excludeRegex,
			ClassLoader classLoader) {
		List<Path> modelPaths = classPathDirectories.stream().flatMap(path -> loadFromClasspath(path, classLoader)).collect(Collectors.toList());
	...
	public static Path findBasicTypesFilePath() {
		return Iterables.getOnlyElement(
				findPathsFromClassPath(ImmutableList.of("model"), ".*\\.rosetta", Optional.empty(), ClassPathUtils.class.getClassLoader()));
	}
	...
	private static Stream<Path> loadFromClasspath(String path, ClassLoader classLoader) {
		...
		try {
			for (URL resource : Collections.list(classLoader.getResources(path))) {
				LOGGER.debug("Got resource {} for path {} ", resource.toString(), path);
